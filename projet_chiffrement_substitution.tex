...
Dans la méthode \texttt{public char chiffrer(char c, int key)} on utilise la clé \textt{key} pour décaler le caractère. \\
...
Dans la méthode \texttt{char dechiffrer(char c, String key)} on renvoie le caractère obtenu en ajoutant 97 à l'indice auquel se trouve le caractère \texttt{c} dans la chaîne \textt{key}. \\
\vspace{1\baselineskip}
